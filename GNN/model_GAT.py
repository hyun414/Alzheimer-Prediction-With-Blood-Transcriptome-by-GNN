...
class GAT(nn.Module):
    def __init__(self, opt, input_dim, omic_dim, label_dim, dropout, alpha, nhids, nheads, fc):
        ...
        fc1 = nn.Sequential(
            ...
            nn.BatchNorm1d(self.fc_dim[0]),  # Batch Normalization 추가
            nn.ELU(),
        ...
        fc5 = nn.Sequential(
            nn.Linear(self.fc_dim[3], omic_dim),
            nn.ELU(),
            nn.AlphaDropout(p=self.dropout, inplace=False)
            )

        self.encoder = nn.Sequential(fc1, fc2, fc3, fc4, fc5)
    ...
    def forward(self, x, adj, grad_labels, opt):
        ...
        batch = batch.unsqueeze(1).repeat(1, x.size(1)).view(-1)
        ...
        # if opt.task == "grad":
        #     one_hot_labels = torch.zeros(grad_labels.shape[0], 2).scatter(1, grad_labels.reshape(-1, 1).long(), 1)
        #     one_out = torch.cat([1 - out, out], dim=1)
        #     y_c = torch.sum(one_hot_labels*one_out)
        # elif opt.task == "surv":
        #     y_c = torch.sum(out)
            
        # print(out.view(-1), one_hot_labels.view(-1), y_c)
...
class GraphAttentionLayer(nn.Module):
    ...
    def forward(self, input, adj):
        ...
        """
        h = torch.matmul(input, self.W)# [bs, N, F]: [batch_size, num_nodes, node_feature_dim]
        bs, N, _ = h.size()

        h = self.bn(h.view(-1, h.size(-1))).view(bs, N, -1)
