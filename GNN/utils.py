...
def load_csv_data(k, opt):
    folder_path = '/work/home/nhkim/BiS495/GNN/input_features_labels/split'
    print("Loading data from:", folder_path+str(k))
    train_data_path = folder_path+str(k)+'_train_features.csv'
    train_label_path = folder_path+str(k)+'_train_labels.csv'
    train_data = np.array(pd.read_csv(train_data_path, header=None))[1:, 1:].astype(float)
    train_label = np.array(pd.read_csv(train_label_path, header=None))[1:,1:].astype(float)

    tr_features = torch.FloatTensor(train_data.reshape(-1, 479, 1)).requires_grad_()
    # tr_features = torch.FloatTensor(train_data.reshape(-1, 274, 1)).requires_grad_()
    tr_labels = torch.LongTensor(train_label)
    print("Training features and labels:", tr_features.shape, tr_labels.shape)

    test_data_path = folder_path+str(k)+'_test_features.csv'
    test_label_path = folder_path+str(k)+'_test_labels.csv'
    test_data = np.array(pd.read_csv(test_data_path, header=None))[1:, 1:].astype(float)
    test_label = np.array(pd.read_csv(test_label_path, header=None))[1:,1:].astype(float)

    te_features = torch.FloatTensor(test_data.reshape(-1, 479, 1)).requires_grad_()
    # te_features = torch.FloatTensor(test_data.reshape(-1, 274, 1)).requires_grad_()
    te_labels = torch.LongTensor(test_label)
    print("Testing features and labels:", te_features.shape, te_labels.shape)

    similarity_matrix = np.array(pd.read_csv(
        '/work/home/nhkim/BiS495/GNN/input_adjacency_matrix/split'+str(k)+'_adjacency_matrix.csv')).astype(float)
